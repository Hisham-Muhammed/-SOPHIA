...
Sophia.tosophia({pattern: 'filter ?(.*)', fromMe: true, desc: Lang.FILTER_DESC, dontAddCommandList: true}, (async (message, match) => {
...
Sophia.tosophia({pattern: 'stop ?(.*)', fromMe: true, desc: Lang.STOP_DESC, dontAddCommandList: true}, (async (message, match) => {
    match = match[1].match(/[\'\"\“](.*?)[\'\"\“]/gsm);
    if (match === null) {
...
Sophia.tosophia({on: 'text', fromMe: false }, (async (message, match) => {
    if(Config.BGMFILTER){
...
async function checkImAdmin(message, user = message.client.user.jid) {
    ...
    return sonuc.includes(true);
     ...
     Sophia.tosophia({on: 'text', fromMe: false,onlyGroup: true}, (async (message, match) => {
...
if(pattern.test(message.message)){
    ...
    filtreler.map(
     ...
     Sophia.tosophia({on: 'text', fromMe: false, onlyPm: true}, (async (message, match) => {
